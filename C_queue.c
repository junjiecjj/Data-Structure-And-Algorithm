...
bool QueueInitial(Queue *queue)//初始化
{
    queue -> head = NULL;
    queue -> tail = NULL;
    queue -> num = 0;
...
bool enqueue(Queue *queue,int value)//入队
{
    Node  *node = (Node *)malloc(sizeof(Node));
    if(node == NULL)
    {
...
bool dequeue(int *x, Queue *queue)//出队
{
    Node *node;
    if(queue -> head == NULL)
        return false;
...
void Traverse(const Queue *queue,void (*pfun)(const Node *node))//遍历队列
{
    Node * pr = queue -> head;
    while(pr != NULL)
    {
